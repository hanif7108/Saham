...
def compute_signals(df: pd.DataFrame) -> dict:
    ...
    # ---- Data sufficiency ----
    # MA50 butuh >= 50 bar, MACD(26,9) butuh ~35 bar agar nilai terakhir valid.
    # Dengan data kurang, indikator (terutama MA50/MACD) bisa menyesatkan.
    n = len(close)
    MIN_BARS_MACD = 35
    MIN_BARS_MA50 = 50
    reliable = n >= MIN_BARS_MACD
    ma50_reliable = n >= MIN_BARS_MA50
    ...
    # MA Cross status — hanya bermakna jika MA50 punya cukup data.
    ma_cross = ("GOLDEN" if last_ma20 > last_ma50 else "DEATH") if ma50_reliable else None
    ...
    # MACD histogram (hanya skor jika data cukup untuk warmup MACD)
    if reliable:
    ...
    # MA Cross + posisi harga (hanya jika MA50 reliabel)
    ...
    if not reliable:
        reasons.append(f"Data terbatas ({n} bar) — indikator MA50/MACD belum reliabel")
    ...
    return {
        ...
        "ma50": round(last_ma50, 2) if ma50_reliable else None,
        "ma_cross": ma_cross,
